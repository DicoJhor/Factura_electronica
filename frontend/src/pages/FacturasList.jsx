...
const FacturasList = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      console.log('🔍 Cargando datos para empresaId:', empresaId);
      
      // Cargar empresa primero
      console.log('📊 Obteniendo datos de empresa...');
      const empresaData = await empresaService.obtenerPorId(empresaId);
      console.log('✅ Empresa obtenida:', empresaData);
      setEmpresa(empresaData);
      
      // Luego cargar facturas
      console.log('📄 Obteniendo facturas...');
      const facturasData = await facturaService.listar(empresaId);
      console.log('✅ Facturas obtenidas:', facturasData);
      ...
      let mensajeError = 'Error al cargar los documentos';
      
      if (error.response?.status === 404) {
        mensajeError = `No se encontró el recurso. Verifica que la ruta ${error.config?.url} exista en el servidor.`;
      ...
      // Si falló obtener facturas pero tenemos empresa, continuar
      if (empresaData) {
        setFacturas([]);
      }
    } finally {
